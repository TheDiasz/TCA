...
def app():
    ...
    selected_box = st.sidebar.selectbox('Escolha um Filtro',('None', 'Brilho', 'Invertido', 'Verão', 'Inverno', 'Alto Contraste', 'Sépia', 'Filtro de Cor'))
    ...
    if selected_box == 'None':
        st.title('Filtros para Imagem')
        ## Add bulletins
        st.subheader("", anchor=None)

        st.subheader("Filtros Disponiveis", anchor=None)

        st.markdown('<ul> <li> Brilho  <li> Invertido<li> Verão   <li> Inverno  <li> Alto Contraste<li> Sépia  <li> Filtro de Cor </ul>', unsafe_allow_html=True)
      


    if selected_box == 'Brilho':
        st.title('Filtro de Brilho')
        image = load_image()
        useWH = st.button('CONVERTER')

        if useWH:    
            resized_image = img2bright(image)
            st.image(resized_image, caption=f"Imagem com o filtro", use_column_width=True)
    ...
    if selected_box == 'Invertido':
        st.title('Inverter Imagem')
        image = load_image()

        useWH = st.button('CONVERTER')
        if useWH:    
            res = img2inv(image)
            st.image(res, caption=f"Imagem com o filtro", use_column_width=True)

    if selected_box == 'Verão':
        st.title('Filtro de Verão')
        image = load_image()
    ...
    if selected_box == 'Inverno':
        st.title('Filtro de Inverno')
        image = load_image()
    ...
    if selected_box == 'Alto Contraste':
        st.title('Filtro de Alto Contraste')
        image = load_image()
    ...
    if selected_box == 'Sépia':
        st.title('Filtro Sépia')
        image = load_image()
    ...
    if selected_box == 'Filtro de Cor':
        st.title('Filtro de Cor') 
        image = load_image()
        ...
        if useWH:
            r1 = img2tone(im1, 0)
            st.image(r1, caption=f"Filtro Vermelho", use_column_width=True)    
            r2 = img2tone(im2, 1)
            st.image(r2, caption=f"Filtro Verde", use_column_width=True)
            r3 = img2tone(im3, 2)
            st.image(r3, caption=f"Filtro Azul", use_column_width=True)
